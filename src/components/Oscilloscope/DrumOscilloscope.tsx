/**
 * Drum Oscilloscope Component
 * Displays real-time waveform visualization of drum output
 */
...
interface DrumOscilloscopeProps {
  drumMachine: DrumMachine | null;
}

export const DrumOscilloscope: React.FC<DrumOscilloscopeProps> = ({ drumMachine }) => {
  ...
  useEffect(() => {
    if (!drumMachine || !canvasRef.current) return;
    ...
    const context = drumMachine.getContext();
    if (context) {
    ...
    const getPeakAmplitude = (data: Float32Array): number => {
      let peak = 0;
      for (let i = 0; i < data.length; i++) {
        const abs = Math.abs(data[i]);
        if (abs > peak) peak = abs;
      }
      return Math.max(peak, 0.01); // Prevent division by zero
    };
    ...
    const drawWaveform = (data: Uint8Array) => {
      ...
      // Display exactly 2 periods or a portion of the buffer
      ...
      // Draw waveform with peak normalization
      const peakAmplitude = getPeakAmplitude(averaged);
      const maxAmplitude = height * 0.35;
      const normalizedScale = peakAmplitude > 0 ? 1.0 / peakAmplitude : 1.0;
      ...
      for (let i = 0; i < displayLength; i += sampleStep) {
        ...
        const value = averaged[srcIdx] * normalizedScale;
      ...
      if (points.length > 1) {
        ...
        // Fill with gradient - using orange/red for drums
        ...
        // Draw main waveform line - using orange for drums
        ctx.beginPath();
    ...
    const draw = () => {
      if (!ctx || !drumMachine) return;

      const waveformData = drumMachine.getWaveformData();
      if (waveformData) {
  ...
  return (
    <div className="oscilloscope-module">
      <div className="module-header">
        <span className="module-title">DRUM WAVEFORM</span>
      </div>
